...
describe('CORTEX_ENDTURN_TIER (4.107.2 lever)', () => {
  it('defaults EndTurn to the essential tier (4.107.3: cell-n-r6 + tb21-k5-n1 confirmed)', () => {
    expect(new BaseToolRegistry({}).getTool('EndTurn')?.discoveryTier).toBe('essential');
    expect(new BaseToolRegistry({ CORTEX_ENDTURN_TIER: '' }).getTool('EndTurn')?.discoveryTier).toBe('essential');
  });
  it('essential lifts EndTurn into the turn-1 set; standard keeps it deferred; junk is ignored', () => {
    ...
    expect(new BaseToolRegistry({ CORTEX_ENDTURN_TIER: 'bogus' }).getTool('EndTurn')?.discoveryTier).toBe('essential');
  });
  it('does not mutate the shared base definition', () => {
    new BaseToolRegistry({ CORTEX_ENDTURN_TIER: 'standard' });
    expect(new BaseToolRegistry({}).getTool('EndTurn')?.discoveryTier).toBe('essential');
  });
});
