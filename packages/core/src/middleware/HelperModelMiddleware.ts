...
PREDICTION: <prediction>`;
  ...
  /**
   * R185 FRAME CANDIDATE AUTHOR — the turn-prediction slot with the roles flipped (R178 §1: "predict the agent, not the human"). Given the task,
   * the pane, the state and the writer's own candidates, the helper (thinking off) proposes `count` GENUINELY DIFFERENT next actions for the
   * Terminus-frame menu; the typed chooser picks among writer + author candidates. One short helper call per turn (~2–4 s, sub-cent).
   * Returns parsed candidates; an unparseable answer returns [] (the menu falls back to the writer's candidates — never worse than the frame).
   */
