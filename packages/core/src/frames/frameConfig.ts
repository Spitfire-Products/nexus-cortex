...
export interface FrameConfig { frame: FrameMode; chooser: ChooserMode; candidates: number; waitCapS: number; screenLines: number; keyGuard: boolean; /** R184: distinct writer candidates the menu step needs before it runs (1 = off; auto 2 when the chooser is on) */ minCandidates: number }
export const FRAME_CONFIG_DEFAULTS: FrameConfig = { frame: 'tools', chooser: 'off', candidates: 1, waitCapS: 60, screenLines: 45, keyGuard: true, minCandidates: 1 };
...
const FRAME_CONTRACT_MENU = (cfg: FrameConfig): string => cfg.minCandidates >= 2
