...
const FRAME_CONTRACT_MENU = (cfg: FrameConfig): string => cfg.minCandidates >= 2 && cfg.author === 'helper'
  ? `\n\nTHE MENU (chooser on): every turn give ${cfg.minCandidates} to ${cfg.candidates} GENUINELY DIFFERENT candidate actions, best first — a different command, a different approach, or a different diagnostic, never variants of one command. If only one action makes sense, give that one plus "single_reason": one sentence saying why; a single candidate without it is sent back unexecuted. A second author may add alternatives, and a harness reader picks the option to press — the executed action may not be your first; the STATE line and the screen always show what ran.`
  : cfg.minCandidates >= 2
