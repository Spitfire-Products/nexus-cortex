...
describe('R179 frameConfig', () => {
  it('defaults to the tools frame with the chooser off; parses and clamps the levers', () => {
    expect(resolveFrameConfig({})).toEqual({ frame: 'tools', chooser: 'off', candidates: 1, waitCapS: 60, screenLines: 45, keyGuard: true, minCandidates: 1, author: 'off', authorModel: '' });
    expect(resolveFrameConfig({ CORTEX_FRAME: 'terminus', CORTEX_FRAME_CHOOSER: 'jev', CORTEX_FRAME_CANDIDATES: '3', CORTEX_FRAME_WAIT_CAP_S: '120', CORTEX_FRAME_SCREEN_LINES: '80', CORTEX_FRAME_KEY_GUARD: 'off' })).toEqual({ frame: 'terminus', chooser: 'jev', candidates: 3, waitCapS: 120, screenLines: 80, keyGuard: false, minCandidates: 2, author: 'helper', authorModel: '' });
    expect(resolveFrameConfig({ CORTEX_FRAME: 'TERMINUS', CORTEX_FRAME_CANDIDATES: '9', CORTEX_FRAME_WAIT_CAP_S: '1', CORTEX_FRAME_CHOOSER: 'bogus' })).toEqual({ frame: 'terminus', chooser: 'off', candidates: 3, waitCapS: 60, screenLines: 45, keyGuard: true, minCandidates: 1, author: 'off', authorModel: '' });
