...
describe('R179 frameConfig', () => {
  it('defaults to the tools frame with the chooser off; parses and clamps the levers', () => {
    expect(resolveFrameConfig({})).toEqual({ frame: 'tools', chooser: 'off', candidates: 1, waitCapS: 300, screenLines: 60 });
    ...
    expect(resolveFrameConfig({ CORTEX_FRAME: 'TERMINUS', CORTEX_FRAME_CANDIDATES: '9', CORTEX_FRAME_WAIT_CAP_S: '1', CORTEX_FRAME_CHOOSER: 'bogus' })).toEqual({ frame: 'terminus', chooser: 'off', candidates: 3, waitCapS: 300, screenLines: 60 });
