...
describe('R185 candidate author in the runner', () => {
  ...
  it('a justified single option is accepted, the author widens it, Jev can pick an authored candidate, and the event banks the author + reason', async () => {
    ...
    const deps = { ...fakeDeps([PROMPT], log), recordEvent: (_k: string, d: any) => events.push(d),
      ...
      jev: async (_s: any, q: any) => { const a: Record<string, number> = {}; for (const k of Object.keys(q)) if (k.startsWith('pick_')) a[k] = k === 'pick_c2' ? 0.9 : 0.2; return a; } };
    ...
    expect(ev.singleReason).toMatch(/only sensible/);
  });
  it('R186: with the author on, an UNJUSTIFIED single option is still handed back — the writer owes the menu', async () => {
    const log: string[] = [];
    const r = new FrameRunner(cfg, { ...fakeDeps([PROMPT], log), author: async () => [{ label: 'x', keystrokes: 'pwd\n', durationS: 2, why: '' }] });
    const res = await r.step({ action: { analysis: 'a', plan: 'p', candidates: [{ label: 'ls', keystrokes: 'ls\n', duration_s: 2 }] }, task: 't', elapsedMs: 0, deadlineMs: 0, cwd: '/app', sessionId: 's', predictorModel: 'gen' });
    expect(res.content).toContain('MENU NEEDS ALTERNATIVES'); expect(res.content).toContain('single_reason'); expect(log.some((l) => l.startsWith('TmuxSession:send'))).toBe(false);
  });
