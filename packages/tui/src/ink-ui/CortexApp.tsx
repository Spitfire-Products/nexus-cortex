...
const EnhancedInput: React.FC<{
  /** prefillProvenance (§5.1): how this submission relates to the ghost prefill —
   *  'none' (never rendered since last submit) | 'shown' (rendered, typed
   *  independently) | 'inserted' (Tab/→-accepted, with or without edits). */
  onSubmit: (text: string, prefillProvenance: 'none' | 'shown' | 'inserted') => void;
  disabled: boolean;
  placeholder?: string;
  ...
  useInput((input, key) => {
    ...
    if (key.return) {
      if (showingSuggestions) {
        ...
        const provenance: 'none' | 'shown' | 'inserted' = ghostInsertedRef.current
          ? 'inserted'
...
export const CortexApp: React.FC<CortexAppProps> = ({
  ...
  const [queuedMessage, setQueuedMessage] = useState<{ text: string; prefillProvenance: 'none' | 'shown' | 'inserted' } | null>(null);
  ...
  useInput((input, key) => {
    ...
    if (key.tab && !key.shift) {
      if (reasoningToggleable) {
        // Toggle extended reasoning baseline <-> high. Honest labeling: on
        // Anthropic interleaved models 'none' maps to the same 10K thinking
        // budget as the default — thinking never fully disables there, so
        // "OFF" would be a lie (audit 2026-07-28).
        setReasoningEffort(prev => {
          const newEffort = prev === 'none' ? 'high' : 'none';
          addItem({
            type: MessageType.Info,
            infoContent: `Extended reasoning: ${newEffort === 'high' ? 'HIGH' : 'baseline'}`,
          }, Date.now());
          return newEffort;
        });
      } else {
  ...
  const handleSubmit = useCallback(async (text: string, prefillProvenance: 'none' | 'shown' | 'inserted' = 'none') => {
