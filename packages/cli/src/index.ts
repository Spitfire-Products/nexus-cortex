...
autoresearch
  ...
  .requiredOption('--experiment-tag <tag>', 'experiment / swarm-member id')
  .requiredOption('--candidate-dir <path|url>', 'checkout/worktree with the candidate fix (gets built + served); a PUBLIC http(s) git URL is shallow-cloned on demand')
  .requiredOption('--task-set <path>', 'TRAIN task-set JSON file or dir (drives keep/discard); a relative path resolves against the candidate checkout')
  .option('--base-dir <path|url>', 'base checkout (default: project root; assumed prebuilt unless --build-base); a PUBLIC http(s) git URL is shallow-cloned on demand')
  .option('--holdout-set <path>', 'HOLDOUT task-set (separate file; required for a mergeable verdict); a relative path resolves against the candidate checkout')
...
autoresearch
  ...
  .requiredOption('--repo <path|url>', 'git repo to improve (worktrees are made off a dedicated loop branch; your branch is untouched); a PUBLIC http(s) git URL is shallow-cloned on demand')
  .requiredOption('--task-set <path>', 'train task-set JSON file or directory; a relative path resolves against the repo checkout')
  .option('--holdout-set <path>', 'holdout task-set — REQUIRED for verified merges (else accepts on train-only, unverified); a relative path resolves against the repo checkout')
  ...
  .option('--model <id>', 'model for cortex targets / the LLM Fixer (arm 1 baseline when --width > 1)')
  .option('--temperature <n>', 'dispatch temperature passed through to each round’s experiment (effectiveness layer)')
  .option('--strategy <label>', 'arm persona/strategy label passed through to each round’s experiment (effectiveness layer; default per-arm fixer:<model> when --width > 1)')
  .option('--width <n>', 'parallel candidate arms per round — each arm gets its own worktree + Fixer model, the gate is FWER-adjusted to the family width, the best accepted arm merges', '1')
  .option('--arm-models <list>', 'comma-separated model ids rotated across arms (arm 1 keeps --model when set)')
  .option('--providers <list>', 'comma-separated providers to draw arm models from (each contributes its flagship tool-supporting model)')
  .option('--missing-provider-key-policy <policy>', 'arm whose provider key is missing: platform_fallback (run anyway — an upstream proxy may fund it) | omit (drop the arm) | redistribute (reassign to a funded model)', 'platform_fallback')
