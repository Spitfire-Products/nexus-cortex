...
Non-breaking. Notable for client authors:

- **`useTable` `isReady` fix** — the React hook's `isReady` flag no longer reverts to `false` after the first row event arrives. If you saw flicker / loading-state regressions on initial subscription, 2.1.0 fixes it. ([#4580](https://github.com/clockworklabs/SpacetimeDB/pull/4580))
- **v2 client subscription stability** — a v2 client disconnecting no longer drops subscriptions belonging to *other* v2 clients. Multi-client setups in the same browser/process are now isolated correctly. ([#4648](https://github.com/clockworklabs/SpacetimeDB/pull/4648))
- **Anonymous view subscription cleanup** fixed (impact only if you use views).
- **Rust client Wasm support** added — irrelevant for TS clients but unblocks Rust-WASM browser clients sharing the same module.

The 2.0 → 2.1 upgrade is API-compatible. `^2.0.2` in package.json already resolves to 2.1.x; run `npm update spacetimedb` if you want the lockfile bump.
...
Server-side projections over tables. Clients subscribe to a view exactly like a table (`SELECT * FROM view_name [WHERE col = 'x']`); the view auto-updates when the data it reads changes.

### Two return-shape families

| Shape | Update model | Output type | Use case |
|---|---|---|---|
| **Query-builder** (`ctx.from.X.where(...).leftSemijoin(...)`) | **Incremental** — engine updates only affected rows | Source row type only | Filtering / joining / scanning many rows |
| **Procedural** (`(ctx) => Array<CustomRow>` or `(ctx) => row \| undefined`) | **Full re-evaluation** on read-set change (black box) | Any product type via `t.row(...)` or `t.option(table.rowType)` | Custom shape (collapse `Option`, compute fields, aggregate counts) |

### ViewContext vs AnonymousViewContext

```typescript
// AnonymousViewContext — same result for everyone, materialized once and shared.
// Strongly preferred when the result doesn't depend on the caller.
...
// ViewContext — result depends on ctx.sender(), materialized per-client.
// At 1k clients, that's 1k materializations + change-tracking sets.
export const my_items = spacetimedb.view(
  { name: 'my_items', public: true },
  t.array(Item.rowType),
  (ctx) => [...ctx.db.item.by_owner.filter(ctx.sender)]
);

// Procedural with custom row shape — collapse joins into a flat structure.
...
export const players_for_level = spacetimedb.anonymousView(
  ...
  (ctx) => {
    ...
    for (const pl of ctx.db.playerLevels.level.filter(2n)) {
      const p = ctx.db.players.id.find(pl.player_id);
      if (p) out.push({ id: p.id, name: p.name, level: pl.level });
    }
    return out;
  }
);
...
Way richer than subscription `WHERE` (which only supports `=`):

| Operation | Method |
|---|---|
| Equal / not-equal | `eq`, `ne` |
| Less / less-or-equal | `lt`, `lte` |
| Greater / greater-or-equal | `gt`, `gte` |
| Boolean | `.and(...)`, `.or(...)`, `.not()` |
| Filter | `.where(p => ...)` (chains AND) |
| Join | `.leftSemijoin(other, (l, r) => ...)`, `.rightSemijoin(other, (l, r) => ...)` |

Join predicates **must use indexed columns**.

### Why `.iter()` is forbidden in procedural views

Procedural view functions are Turing-complete black boxes. The engine tracks the "read set" (rows the function read) and re-runs the entire function when any row in that set changes. A `.iter()` call puts every row in the read set — any write triggers full re-evaluation. With millions of rows, this is prohibitive.

Index lookups (`.find()`, `.filter()` on indexed columns) give the engine a precise read-set, so re-evaluation only fires when a relevant row changes. `count()` is exempt (constant-time metadata).

For "give me all rows" patterns, use a query-builder view — incremental even on full scans.
...
```typescript
conn.subscriptionBuilder().subscribe('SELECT * FROM high_scorers');
conn.subscriptionBuilder().subscribe("SELECT * FROM player_and_level WHERE id = 2");

// Query-builder views with primary-key inferable row types get onUpdate callbacks;
// otherwise insert/delete-only.
conn.db.highScorers.onInsert((ctx, row) => { ... });
conn.db.highScorers.onUpdate((ctx, oldRow, newRow) => { ... });
```

**Subscription `WHERE` on a view inherits the standard subscription limits** (only `=`, can't filter `Option<T>` fields). View-internal `.where()` clauses use the richer query-builder operators.

### Workaround: shadow-index table for `Option<T>` subscription filtering

Subscriptions can't filter on `Option<T>` columns. If you have a denormalized `Option<string>` column added for room/scope filtering that turned out to be unfilterable, the canonical workaround is a parallel public table whose scope column is non-null from creation.

Server-side: dual-write to the index table inside the same reducer transaction. Client-side: subscribe to the index instead of the source. The index carries enough fields to render UI without joining back. See `spacetimedb-rust` skill for the full pattern.

**Why not a view for this?** Query-builder views can't transform the source's `Option<T>` into `T` (output is the source row type). Procedural views can transform shape but require full re-eval on every write and forbid `.iter()`. Index tables give incremental updates AND non-null projection.
