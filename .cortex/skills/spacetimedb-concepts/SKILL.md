...
2. **Backend:** Define reducer(s) to mutate the data (include `register_identity` + `sync_user_tier` if joining the platform)
...
### Subscription Query Limitations (2.0/2.1)

The subscription engine is intentionally narrow — only what can be incrementally evaluated efficiently:

- **WHERE supports `=` only.** `!=`, `<`, `>`, `<=`, `>=` will crash the subscription. For richer comparisons, define a [view](#views) instead — view query-builders support full `eq/ne/lt/lte/gt/gte/and/or/not`.
- **`Option<T>` columns can't be filtered server-side.** Even `column = 'x'` fails when `column: Option<String>`. Subscribe unfiltered + filter client-side, OR use the shadow-index workaround (see Rust skill — define a parallel public table with `String` instead of `Option<String>` and dual-write).
- **No `IN (...)`** — emit one query per key or rethink the scope.
- **No `JOIN`** in subscription queries — joins live in views.
- **One bad query in a batch fails the whole subscription** — submit each query separately if uncertain.

These limits exist because subscriptions get incremental updates: when a row changes, the engine computes the delta to your output without re-running the whole query. Operators that don't fit the differential model (unbounded scans, complex predicates) move to views, which can either be query-builder (incremental, source row type only) or procedural (full re-eval, custom shape).

## Views

Views are read-only server-side projections over tables. Clients subscribe to a view exactly like a table (`SELECT * FROM view_name [WHERE col = 'x']`), and the view automatically updates when the data it reads changes.

Two return shapes with very different cost profiles:

| Shape | Update model | Output type | Iteration |
|---|---|---|---|
| Query-builder (`Query<T>` / `IQuery<T>` / TS array of source row) | **Incremental** — engine analyzes plan, updates only affected rows | Source row type only | Engine handles full table scans |
| Procedural (`Vec<T>` / `Option<T>` / TS array of custom row) | **Full re-eval** when read-set changes | Any registered type (custom shape) | **Indexed lookups only** — `.iter()` forbidden |

Two context types orthogonal to shape:

- **`AnonymousViewContext`** — same result for everyone, materialized once and shared. Strongly preferred when the result doesn't depend on the caller.
- **`ViewContext`** — result depends on `ctx.sender()`, materialized separately for each subscriber. Per-client compute cost.

### Why Procedural Views Forbid `.iter()`

Procedural views are Turing-complete black boxes. The engine tracks the "read set" (which rows the view function read) and re-runs the entire function when any row in that set changes. A `.iter()` call puts every row of the table in the read set, so any write triggers full re-evaluation. With millions of rows, that's prohibitive.

Index lookups (`.find()`, `.filter()` on indexed columns) give the engine a precise read-set, so re-evaluation only fires when a relevant row changes. `count()` is also allowed (constant-time metadata).

For "give me all rows" patterns, use a query-builder view — the engine can analyze the plan and update incrementally even on full-table scans.

### When to Use Views vs Subscriptions vs Reducers

| Need | Use |
|---|---|
| Real-time read of a table or scoped subset | Subscription |
| Filter beyond `=`, server-side joins, computed shape | View (query-builder for incremental, procedural for custom shape) |
| Per-user "my X" view scoped by sender | `ViewContext` view |
| Shared materialization across all clients | `AnonymousViewContext` view |
| Mutate state | Reducer |
