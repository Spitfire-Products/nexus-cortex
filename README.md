...
- **Every major provider, one harness.** The five major labs — **Anthropic, OpenAI, Google/Gemini, xAI, and DeepSeek** — are proven end-to-end. A dozen more (Cloudflare Workers AI, Zhipu/GLM, Qwen, Moonshot/Kimi, MiniMax, Mercury) are wired through the same adapter layer. <!--AUTO-COUNT:models-->102<!--/AUTO-COUNT--> models across <!--AUTO-COUNT:providers-->12<!--/AUTO-COUNT--> providers in all — switch mid-session, route from benchmark history, or mix providers across sub-agents. Run `cortex models list` for the live set.
...
- **An embeddable engine, not a closed app.** The orchestrator, adapters, <!--AUTO-COUNT:tools-->51<!--/AUTO-COUNT--> built-in tools, and middleware are a clean TypeScript library you build on.
...
- **Portable agent memory (canon).** Sessions live in a provider-neutral canonical JSONL store — the pattern the field now calls *portable agent memory* / *cross-harness handoff*. History is written once, lossless and append-only, and translated per provider at the edge: switch models mid-session, keep prompt caches warm, and carry context across harnesses. A git-backed backend (`GitHistoryStore`) makes that store a repo you own, and it keeps itself current: `cortex config set CANON_AUTO_SYNC true` syncs after every turn (debounced), `cortex canon watch` captures sessions any other harness writes on disk. See **[Canon](docs/CANON.md)**.
